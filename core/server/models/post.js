...
Post = ghostBookshelf.Model.extend({
    ...
    updateTags: function (newTags, attr, options) {
        ...
        //switched to slug for retrieval as SQLITE is returning 0 for id
        //after trigger execution.
        return Post.forge({slug: this.get('slug')}).fetch({withRelated: ['tags'], transacting: options.transacting}).then(function (thisPostWithTags) {
    ...
    search: function (opts) {
        var postCollection,
            permittedOptions = ['page', 'limit', 'status', 'staticPages', 'match'];
        ...
        // aren't used for other purposes.
        return postCollection
            .query('where', 'markdown', 'MATCH', opts.match)
            .query('limit', opts.limit)
            .query('offset', opts.limit * (opts.page - 1))
            .query('orderBy', 'status', 'ASC')
            .query('orderBy', 'published_at', 'DESC')
    ...
    add: function (newPostData, options) {
        ...
        return ghostBookshelf.Model.add.call(this, newPostData, options).then(function (post) {
            ...
            return when(post.updateTags(newPostData.tags, null, options)).then(function () {
                //Switched to slug for retrieval. SQLITE is returning 0 for id in trigger
                return self.findOne({status: 'all', slug: post.get('slug')}, options);
            });
        });
    },
